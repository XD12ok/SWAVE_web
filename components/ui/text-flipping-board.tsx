...
const FlapCell = React.memo(
  function FlapCell({
    ...
    const topBg = accent?.top ?? "bg-neutral-900 dark:bg-neutral-900";
    const bottomBg = accent?.bottom ?? "bg-neutral-900 dark:bg-neutral-900";
    const textColor = accent?.text ?? "text-neutral-100 dark:text-white";

    const flapTopBg = prevAccent?.top ?? "bg-neutral-100 dark:bg-neutral-800";
    const flapTextColor =
      prevAccent?.text ?? "text-neutral-900 dark:text-white";

    const bottomDelay = flipDuration * 0.5;

    return (
      <div className="flex aspect-3/6 flex-col overflow-hidden rounded-[2px] border border-black md:rounded-[3px] md:border-2 dark:border-black">
        {/* Flap content area */}
        <div className="relative flex-1 perspective-dramatic transform-3d">
          <div className="absolute inset-0 z-40 hidden flex-row items-center justify-center md:flex">
            <div className="h-1/2 w-px rounded-tr-sm rounded-br-sm bg-black dark:bg-black" />
            <div className="flex h-px flex-1 bg-black dark:bg-black" />
            <div className="h-1/2 w-px rounded-tl-sm rounded-bl-sm bg-black dark:bg-black" />
          </div>
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-full rounded-xl bg-neutral-900 p-2 shadow-xl md:rounded-2xl md:p-4 dark:bg-neutral-900 dark:shadow-[0_20px_70px_-15px_rgba(0,0,0,0.6)]",
        className,
      )}
    >
      <div
